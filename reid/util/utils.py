...
def show_alignedreid(local_img_path, img_path, dist):
    ...
    index1 = local_img_path.index('p')
    index2 = img_path.index('p')
    picturename = local_img_path[7:index1 - 1] + 'vs' + img_path[7:index2 - 1]
    plt.savefig('D:/Someprograms/AlignedReID-master/result/{}.png'.format(picturename))
...
def drawmyresut(local_img_path, img_path, dist):
    img1 = cv2.imread(local_img_path)
    img2 = cv2.imread(img_path)
    img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2RGB)
    img2 = cv2.cvtColor(img2, cv2.COLOR_BGR2RGB)
    ...
    plt.imshow(img2)
    picturename = local_img_path[7:index1-1]+'vs'+img_path[7:index2-1]
    # plt.subplot(1,2,1).set_title(local_img_path[7:index1-1])
    plt.title('Aligned distance: %.4f \n Original distance: %.4f' % (d, origin_dist))
    # plt.imshow(img1)
    # plt.subplot(1,2,2).set_title(img_path[7:index2-1])
    # plt.imshow(img2)
    # plt.show()
    plt.savefig('D:/Someprograms/AlignedReID-master/result/{}.png'.format(picturename))