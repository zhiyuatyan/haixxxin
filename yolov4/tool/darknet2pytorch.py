...
import numpy as np
from yolov4.tool.region_loss import RegionLoss
...
class Darknet(nn.Module):
        ...
        for block in blocks:
            if block['type'] == 'net':
                ...
                if batch_normalize:
                    ...
                    # siamprn++.add_module('bn{0}'.format(conv_id), BN2d(filters))
                else:
                    model.add_module('conv{0}'.format(conv_id),
    ...
    #             siamprn++ = self.models[ind]
    #             batch_normalize = int(block['batch_normalize'])
    #             if batch_normalize:
    #                 save_conv_bn(fp, siamprn++[0], siamprn++[1])
    #             else:
    #                 save_conv(fp, siamprn++[0])
    #         elif block['type'] == 'connected':
    #             siamprn++ = self.models[ind]
    #             if block['activation'] != 'linear':
    #                 save_fc(fc, siamprn++)
    #             else:
    #                 save_fc(fc, siamprn++[0])
    #         elif block['type'] == 'maxpool':
    #             pass
    #         elif block['type'] == 'reorg':
    #             pass
