...
def init_model(config, model, logger, optimizer=None, lr_scheduler=None):
    """
    load siamprn++ from checkpoint or pretrained_model
    """
    ...
    if checkpoints:
        ...
        for idx, pretrained in enumerate(pretrained_model):
            ...
            logger.info("load pretrained siamprn++ from {}".format(
                pretrained_model))
    else:
...
def save_model(net,
               ...
               **kwargs):
    """
    save siamprn++ to the target path
    """
    ...
    if is_best:
        logger.info('save best siamprn++ is to {}'.format(model_prefix))
    else:
        logger.info("save siamprn++ in {}".format(model_prefix))
