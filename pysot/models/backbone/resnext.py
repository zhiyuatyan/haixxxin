...
class Bottleneck_CI(nn.Module):
    """
    Bottleneck with center crop layer, utilized in CVPR2019 siamprn++
    """
    expansion = 4
...
class ResNeXt(nn.Module):
    ...
    """
    def __init__(self, num_blocks, cardinality, bottleneck_width, used_layers, expansion=2):
    ...
    def forward(self, x):
        x_ = F.relu(self.bn0(self.conv0(x)))
        x = self.maxpool1(x_)
...
def resnext22(**kwargs):
    model = ResNeXt(num_blocks=[3, 4], cardinality=32, bottleneck_width=4, **kwargs)
    return model